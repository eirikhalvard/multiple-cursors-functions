  ...
  activate() {
    ...
    this.subscriptions.add(
      atom.commands.add("atom-workspace", {
        ...
        "multiple-cursors-functions:split-selections": () => splitSelections(),
...
function selectionsToCursors() {
  ...
  selections.forEach(selection => {
    range = selection.getScreenRange();
    selection.clear();
    // not yet implemented! should just be a for-loop that does:
    // editor.addCursorAtBufferPosition(someRange);
  });
}
